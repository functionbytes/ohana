                ...
                <form id="formNotes" enctype="multipart/form-data" role="form" onSubmit="return false">
                    ...
                    <div class="card-body border-top">
                        <div class="d-flex no-block align-items-center">
                            ...
                            <h5 class="mb-0 uppercase">Editar nota # {{$note->number}}
                            </h5>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 mb-3 d-none container-visit">
                                ...
                                <input type="text" class="form-control picker" name="visit" value="{{ $note->status_id == 1 ? $note->visit_at_formatted : '' }}" autocomplete="off">
                            </div>

                            <div class="col-6 mb-3 d-none container-nextcall">
                                <label class="control-label col-form-label">Reprogramar llamada</label>
                                <input type="text" class="form-control picker" name="nextcall" value="{{ $note->status_id == 2 && $last ? $last->next_call_at_formatted : '' }}" autocomplete="new-password">
                            </div>

                            <div class="col-12 mb-3 d-none container-nextcall">
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            function toggleVisitFields(isInit = false) {
                const status = $('#status').val();
                const statusId = '{{ $note->status_id }}';
                const scheduleId = '{{ $note->schedule_id }}';
                //const visitAt = '{{ $note->visit_at_formatted }}';

                function resetFields(selector) {
                    $(selector).addClass('d-none').find('input, textarea').val('');
                    $(selector).find('select').val(null).trigger('change');
                }

                if (!isInit) {
                    resetFields('.container-schedule');
                    resetFields('.container-visit');
                    resetFields('.container-nextcall');
                    resetFields('.container-notes');
                }
                ...
                if (status === '1') {
                    ...
                    if (isInit && statusId === '1') {
                        $('#schedule').val(scheduleId).trigger('change');
                    }
                }
                ...
                if (status === '3') {
                    $('.container-notes').removeClass('d-none');
                } else {
                    $('.container-notes').addClass('d-none').find('textarea').val('');
                }
            }


            toggleVisitFields(true);

            $('#status').on('change', function () {
                toggleVisitFields(false);
            });
